...
public class SecurityConfig {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http, RememberMeServices rememberMeServices) throws Exception {
        http// .authorizeHttpRequests(authorize -> authorize
            ...
            // .formLogin(Customizer.withDefaults()); // 시큐리티가 제공하는 기본 로그인 폼 페이지
                .authorizeHttpRequests(authorize -> authorize.requestMatchers("/css/**", "/js/**", "/image/**")
    ...
    // .username("user")
    // .password("{bcrypt}$2a$10$Nsc6BdGPZvEJir9/YC8wkufSQ0/IYgvNfqVMovG.ZEHYbqzFdmrES")
    // .roles("USER") // ROLE_권한명
    // .build();
